...
DROP DATABASE IF EXISTS `IUtest`;
CREATE DATABASE IF NOT EXISTS `IUtest` DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
USE `IUtest`;

--
-- DAMOS PERMISO USO Y BORRAMOS EL USUARIO QUE QUEREMOS CREAR POR SI EXISTE
--
GRANT USAGE ON * . * TO `IUtest`@`localhost`;
...
GRANT ALL PRIVILEGES ON `IUtest`.* TO `iuuser`@`localhost` WITH GRANT OPTION;
